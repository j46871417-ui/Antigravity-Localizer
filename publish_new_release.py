...
subprocess.run(["git", "commit", "-m", f"chore: release v{new_tag} (auto process termination on Next + clean installer close)"], cwd=repo_root, check=False)
...
- **Устранение конфликта заблокированных файлов**:
  - При нажатии кнопки «Далее» установщик автоматически проверяет, запущены ли процессы Google Antigravity.
  - Пользователю выводится понятный диалог подтверждения перед закрытием.
  - При согласии все фоновые процессы Electron/Antigravity принудительно и тихо закрываются, исключая ошибки доступа.
- **Корректная последовательность запуска**:
  - Окно установщика Inno Setup теперь **полностью закрывается** сразу после завершения распаковки.
  - Окно русификатора открывается строго **после закрытия установщика**, на экране не остаётся лишних висящих окон.
- **100% русификация интерфейса и ошибок**:
  - Полный перевод составных ссылочных блоков: «См. наше [руководство по устранению неполадок] для получения помощи»
  - Ошибки агента: «Агент остановлен из-за ошибки», «Повторить попытку», «Скопировать данные отладки»
  - Экраны входа, селектор моделей («Экономный», «Средний», «Высокая точность») и все настройки.
...
2. Запустите, нажмите «Далее» — установщик корректно закроет Antigravity, распакует файлы, закроется сам и откроет окно русификатора.
"""
...
• <b>Автоматическое закрытие Antigravity</b>: при нажатии «Далее» установщик выводит подтверждение и чисто закрывает все процессы Antigravity, предотвращая конфликт занятых файлов
• <b>Чистое закрытие установщика</b>: окно установщика Inno Setup автоматически закрывается сразу после распаковки, и только затем открывается окно русификатора (нет наложения окон)
• <b>Исправление составных ссылок</b>: полностью переведены фразы со ссылками («См. наше [руководство по устранению неполадок] для получения помощи»)
• <b>Полная русификация всех экранов и уровней моделей</b>: «Экономный», «Средний», «Высокая точность»
...
💾 <b>Установщик: <code>{installer_exe_name}</code> прикреплён ниже 👇</b>"""
        ...
        if os.path.exists(installer_exe_path):
            ...
            tg_client.send_document(
                installer_exe_path,
                caption=f"🚀 <b>{installer_exe_name}</b>\n(Автозакрытие процессов при нажатии «Далее» + чистое закрытие инсталлятора)"
            )
